...
def http2_settings(body=None):
	return "token64" # default value 

#  ['A-IM', 'Accept', 'Accept-Charset', 'Accept-Datetime', 'Accept-Encoding', 'Accept-Language', 'Access-Control-Request-Headers', 'Access-Control-Request-Method', 'Authorization', 'Cache-Control', 'Connection', 'Content-Encoding', 'Content-Length', 'Content-MD5', 'Content-Type', 'Cookie', 'Date', 'Expect', 'Forwarded', 'From', 'HTTP2-Settings', 'Host', 'If-Match', 'If-Modified-Since', 'If-None-Match', 'If-Range', 'If-Unmodified-Since', 'Max-Forwards', 'Origin', 'Pragma', 'Prefer', 'Proxy-Authorization', 'Range', 'Referer', 'TE', 'Trailer', 'Transfer-Encoding', 'Upgrade', 'User-Agent', 'Via', 'Warning']

def if_match(body=None):
	return "737060cd8c284d8af7ad3082f209582d"

def if_modified_since(body=None):

	return "Sat, 29 Oct 1994 19:43:31 GMT"

def if_none_match(body=None):
	return "737060cd8c284d8af7ad3082f209582d"

def if_range(body=None):
	return "737060cd8c284d8af7ad3082f209582d"

def if_unmodified_since(body=None):
	return "Sat, 29 Oct 1994 19:43:31 GMT"


def max_forwards(body=None):
	return str(random.randrange(-1000000000,1000000000))


def origin(body=None):
	return "127.0.0.1"

def pragma(body=None):
	return "no-cache"


def prefer(body=None):
	return "return=representation"

header_handlers = [aim, accept, accept_charset, accept_datetime, accept_encoding, accept_language,access_control_request_headers,access_control_request_method, authorization, cache_control, connection, content_encoding, content_length, content_md5, content_type, cookie, date, expect, forwarded, from_header, http2_settings, host, if_match, if_modified_since, if_none_match, if_range, if_unmodified_since, max_forwards, origin, pragma, prefer, proxy_authorization, range_header, referer, te, trailer, transfer_encoding, upgrade, user_agent, via, warning]
